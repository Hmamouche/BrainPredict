...
if __name__ == '__main__':
	...
	saccades = resampling. resample_ts (saccades, index, mode = "sum")[:, 1:]
	...
	movement_quantity = resampling. resample_ts (movement_quantity, index, mode = "energy", rotation = False, pixel = True)[:, 1:]
	...
	mean_x_y_gradient = resampling. resample_ts (mean_x_y_gradient, index, mode = "mean")

	# resample facial-time-series: face, mouth and eyes looks
	face_time_series = resampling. resample_ts (np. array (face_time_series), index, mode = "sum")[:, 1:]
