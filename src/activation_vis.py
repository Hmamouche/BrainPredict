...
#=================================================================#
def add_activation (areas_labels, brain_objs, annot_file_R, annot_file_L, annot_data):
    select = []
    ...
    for i in range (len (brain_objs)):
        #brain_objs[i] = BrainObj (name = 'inflated', hemisphere='both', translucent=False, cbtxtsz = 10., verbose = None)
        #brain_objs[i]. clean ()
        #brain_objs[i]. update ()
        if len (left_areas) > 0:
...
if __name__ == '__main__':
    ...
    for i in range (predictions. shape [0]):
        ...
        add_activation (activated_areas_labels, br_objs, r_file, l_file, annot_l)
